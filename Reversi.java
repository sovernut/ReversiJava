...
class Reversi {
    ...
    static void conquer(int r, int c) {
        ...
        try {
            ...
            for (int i = 0; i < 8; i++) {
                int indexXWithDir = row + indexX[i];
                int indexYWithDir = col + indexY[i];
                // System.out.println("indexXWithDir :: "+indexXWithDir);
                // System.out.println("indexYWithDir :: "+indexYWithDir);
                if ((indexXWithDir < 0 || indexXWithDir > 7) || (indexYWithDir > 7 || indexYWithDir < 0)) {
                    System.out.println("CONTINUE");
                    continue;
                }
                // System.out.println("boards[indexXWithDir][indexYWithDir].getName() ::: "
                // + boards[indexXWithDir][indexYWithDir].getName());
                // System.out.println("getTurnName() ::: " + getTurnName());
                if (boards[indexXWithDir][indexYWithDir] != null
                        && boards[indexXWithDir][indexYWithDir].getName() != getTurnName()) {
                    System.out.println("Existing!!!");
                    checkNextPiece(indexXWithDir, indexYWithDir, i);
                }
            }
        } catch (Exception e) {
            System.out.println("Conquer has errored :: " + e);
        }
    }

    static void checkNextPiece(int row, int col, int direction) {
        Piece[][] boards = board.getBoard();
        int count = 0;
        switch (direction) {
            // UPPER LEFT
            case 0:
                ...
                while (true) {
                    if (boards[row][col] == null) {
                        break;
                    } else if (boards[row][col].getName() == getTurnName()) {
                        System.out.println("CASE UPPER LEFT FOUND");
                        for (int j = count; j > 0; j--) {
                            row++;
                            col++;
                            boards[row][col].setNameAndSymbol(getTurnName());
                        }
                        break;
                    } else if (row == 0 || row == 7 || col == 0 || col == 8) {
                        break;
                    } else {
                        count++;
                    }
            ...
            // UPPER MID
            case 1:
                System.out.println("CASE UPPER MID");
                while (true) {
                    if (boards[row][col] == null) {
                        break;
                    } else if (boards[row][col].getName() == getTurnName()) {
                        System.out.println("CASE UPPER MID FOUND");
                        for (int j = count; j > 0; j--) {
                            row++;
                            boards[row][col].setNameAndSymbol(getTurnName());
                        }
            ...
            case 2:
                ...
                while (true) {
                    if (boards[row][col] == null) {
                        ...
                        System.out.println("CASE UPPER RIGHT FOUND");
                        for (int j = count; j > 0; j--) {
                            row++;
                            col--;
                            boards[row][col].setNameAndSymbol(getTurnName());
            ...
            // MIDDLE LEFT
            case 3:
                System.out.println("CASE MIDDLE LEFT");
                while (true) {
                    if (boards[row][col] == null) {
                        break;
                    } else if (boards[row][col].getName() == getTurnName()) {
                        System.out.println("CASE MIDDLE LEFT FOUND");
                        for (int j = count; j > 0; j--) {
                            col++;
                            boards[row][col].setNameAndSymbol(getTurnName());
                        }
            ...
            // MIDDLE RIGHT
            case 4:
                System.out.println("CASE MIDDLE RIGHT");
                while (true) {
                    if (boards[row][col] == null) {
                        break;
                    } else if (boards[row][col].getName() == getTurnName()) {
                        System.out.println("CASE MIDDLE RIGHT FOUND");
                        for (int j = count; j > 0; j--) {
                            col--;
                            boards[row][col].setNameAndSymbol(getTurnName());
                        }
            ...
            case 5:
                ...
                while (true) {
                    if (boards[row][col] == null) {
                        ...
                        System.out.println("CASE LOWER LEFT FOUND");
                        for (int j = count; j > 0; j--) {
                            row--;
                            col++;
                            boards[row][col].setNameAndSymbol(getTurnName());
            ...
            case 6:
                ...
                while (true) {
                    if (boards[row][col] == null) {
                        ...
                        System.out.println("CASE LOWER MID FOUND");
                        for (int j = count; j > 0; j--) {
                            row--;
                            boards[row][col].setNameAndSymbol(getTurnName());
                        }
            ...
            case 7:
                ...
                while (true) {
                    if (boards[row][col] == null) {
                        ...
                        System.out.println("CASE LOWER RIGHT FOUND");
                        for (int j = count; j > 0; j--) {
                            row--;
                            col--;
                            boards[row][col].setNameAndSymbol(getTurnName());
    ...
    static void markPlaceable(String[][] marker, String turn, int x_p, int y_p) {
        ...
        for (int i = 0; i < x.length; i++) {
            ...
            boolean isAllyPieceOpposite = isAllyPieceOpposite(turn, x_p + x[i] * -1, y_p + y[i] * -1, x[i] * -1,
                    y[i] * -1);
